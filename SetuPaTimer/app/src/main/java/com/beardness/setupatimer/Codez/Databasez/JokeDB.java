...
public class JokeDB extends SQLiteOpenHelper {
  ...
  public void onCreate(SQLiteDatabase db) {
    ...
    initialInsertRow(db, "Если бы вагина Ани была видеоигрой", "То у неё бы было 1 место в рейтингах", STATUS_NEW, false);
    ...
    initialInsertRow(db, "Почему у карандаша нет детей?", "Потому, что у него на конце резинка", STATUS_NEW, false);
    initialInsertRow(db, "Хан Тим покинул свою команду и стал:", "Ханом Соло", STATUS_NEW, false);
    ...
    initialInsertRow(db, "Как называют черную женщину сделавшую 6 абортов?", "Борец с преступностью", STATUS_NEW, false);
    initialInsertRow(db, "Почему Буратино хочет на Кавказ?", "Потому что там могут вырезать семью", STATUS_NEW, false);
    ...
    initialInsertRow(db, "Что говорят про некрофила-зануду?", "За**ет мертвого", STATUS_NEW, false);
    ...
    initialInsertRow(db, "Как называется собрание геев?", "СходОчка", STATUS_NEW, false);
    ...
    initialInsertRow(db, "Почему женщинам в тюрьме проще чем мужчинам?", "2 слота под инвентарь", STATUS_NEW, false);
